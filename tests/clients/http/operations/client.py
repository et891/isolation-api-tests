...
class OperationsHTTPTestClient(HTTPTestClient):
    ...
    def get_operations_api(self, query: GetOperationsQueryTestSchema) -> Response:
        return self.get(
            f"{APITestRoutes.OPERATIONS}", query = query.model_dump(by_alias=True, exclude_none=True)
        )
    ...
    def get_operations(self, user_id: uuid.UUID, card_id: uuid.UUID | None = None, account_id: uuid.UUID | None = None) -> GetOperationsResponseTestSchema:
